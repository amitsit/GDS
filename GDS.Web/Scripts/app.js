...
function enableDisableSection(id)
{
    if (id > 0) {
...
function validateItemPricePrecision() {

    $('.toFix').keypress(function (e) {
        if ((e.which != 46 || $(this).val().indexOf('.') != -1) && e.which != 08 && e.which != 0 && (e.which < 48 || e.which > 57)) {
            e.preventDefault();
        }
    });

    $(".toFix").on("keyup", function () {
        var number = ($(this).val().split('.'));
        if (number[0].length > 7) {
            $(this).val(number[0].substr(0, 7));
        }

        if (number.length > 1) {
            if (number[1].length > 2) {
                $(this).val(parseFloat(number[0] + '.' + number[1].substr(0, 2)));
            }
        }
    });
}